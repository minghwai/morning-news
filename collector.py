...
#: 네이버는 검색 API를 개발자센터에서 네이버클라우드 API HUB로 이관했다.
#: 주소와 인증 헤더가 서로 다르므로 두 방식을 모두 지원하고,
#: HUB를 먼저 시도한 뒤 인증에 실패하면 구(舊) 방식으로 자동 전환한다.
HUB_URL = "https://naverapihub.apigw.ntruss.com/search/v1/news"
LEGACY_URL = "https://openapi.naver.com/v1/search/news.json"

_MODE_LABEL = {"hub": "NAVER API HUB", "legacy": "개발자센터(구 방식)"}
...
def _request(mode: str, keyword: str, start: int, client_id: str, client_secret: str):
    if mode == "hub":
        url = HUB_URL
        headers = {
...
def resolve_mode(keyword: str, client_id: str, client_secret: str) -> tuple[str, list[dict]]:
    """HUB를 먼저 시도하고, 인증이 거부되면 구 방식으로 전환한다."""
    last = None
    for mode in ("hub", "legacy"):
        try:
            resp = _request(mode, keyword, 1, client_id, client_secret)
        except Exception as exc:
            last = f"{_MODE_LABEL[mode]} 연결 실패: {exc}"
            continue

        if resp.status_code in (401, 403):
            last = f"{_MODE_LABEL[mode]} 인증 거부 ({resp.status_code})"
            continue
        if resp.status_code == 429:
            raise CollectorError(
                f"{_MODE_LABEL[mode]} 호출 한도 초과(429). "
                "HUB 콘솔의 Application에서 '검색' 서비스가 선택돼 있는지 확인하세요."
            )
        if resp.status_code >= 400:
            last = f"{_MODE_LABEL[mode]} 응답 오류 {resp.status_code}"
            continue

        print(f"      → 인증 방식: {_MODE_LABEL[mode]}")
        return mode, resp.json().get("items", [])

    raise CollectorError(
        "네이버 검색 API 인증에 실패했습니다. 마지막 응답: "
        f"{last}. HUB 키라면 콘솔에서 '검색' 서비스 선택 여부를, "
        "구 개발자센터 키라면 앱에 '검색' API가 추가돼 있는지 확인하세요."
    )


def _fetch_page(mode: str, keyword: str, start: int, client_id: str, client_secret: str) -> list[dict]:
    resp = _request(mode, keyword, start, client_id, client_secret)
    if resp.status_code == 429:
        raise CollectorError(f"{_MODE_LABEL[mode]} 호출 한도 초과(429).")
...
def collect(keyword: str, max_articles: int = 30, hours_back: int = 0) -> dict:
    ...
    mode, first_items = resolve_mode(keyword, client_id, client_secret)

    for start in (1, 101, 201, 301, 401):
        try:
            items = first_items if start == 1 else _fetch_page(
                mode, keyword, start, client_id, client_secret
            )
